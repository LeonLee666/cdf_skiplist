...
static struct skipnode *skipnode_new(int level, float key, float value) {
...
static struct skipnode *skiplist_search(struct skiplist *list, float key) {
    struct skipnode *node;
    int i = list->level - 1;
...
skiplist_insert(struct skiplist *list, float key, float value, int level) {
#ifdef BASE
    level = random_level();
#endif
    if (level > list->level) {
...
static void skiplist_remove(struct skiplist *list, float key) {
    struct sk_link *n;
    struct skipnode *node;
    int i = list->level - 1;
    struct sk_link *pos = &list->head[i];
