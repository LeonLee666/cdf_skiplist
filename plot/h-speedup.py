...
def ReadFile():
  ...
  bounds = []
  partitions = []
  with open('h-speedup.csv') as csvfile:
    rows = csv.reader(csvfile, delimiter = ',')
    
    count = 0
    for row in rows:
      parameters.append(int(row[0]))
      standards.append(float(row[1]))
      partitions.append(float(row[2]))

  lines = [standards, partitions]
...
if __name__ == "__main__":
  ...
  # draw line chart
  legend_labels = ['hot-list','skiplist']
  parameters, lines = ReadFile()

  DrawFigure(parameters, lines, legend_labels, 'h', 'Speedup', 5, 30
, 0.4, 1.8, figure_folder, 'h-speedup', True)
