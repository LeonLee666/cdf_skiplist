...
def ReadFile():
  ...
  bounds = []
  partitions = []
  with open('uniform-speedup.csv') as csvfile:
...
def DrawFigure(x_values, y_values, legend_labels, x_label, y_label, x_min, x_max, y_min, y_max, folder, filename, allow_legend, color_map = COLOR_MAP):
  ...
  ax.xaxis.set_major_locator(plt.LogLocator(2))
...
if __name__ == "__main__":
  ...
  DrawFigure(parameters, lines, legend_labels, 'Dataset Size', 'QPS [W/s]', int(2**14), int(2**25)
, 0.8, 2, figure_folder, 'uniform-speedup', True)
